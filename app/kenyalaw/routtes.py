...
class GPTRelevanceCalculator:
    ...
    async def calculate_gpt_relevance(self, search_term: str, case_facts: str, case_title: str = "") -> Dict:
        ...
        prompt = f"""Analyze the relevance of this case to the search query for Kenyan legal research.

Search Query: "{search_term}"
Case Title: "{case_title}"
Case Facts: "{case_facts[:2000]}"  # Limit for cost control

Rate relevance from 0.0 to 1.0 and explain why:
- 0.8-1.0: Highly relevant (direct match, same legal principles)
- 0.5-0.79: Moderately relevant (related legal area, similar facts)
- 0.2-0.49: Somewhat relevant (tangential connection)
- 0.0-0.19: Not relevant

Respond in JSON format:
{{"relevance_score": 0.85, "reasoning": "Brief explanation of relevance"}}"""

        try:
            response = await self.client.chat.completions.create(
                model="gpt-4o-mini",  # Cheaper option
                messages=[
                    {"role": "system", "content": "You are a Kenyan legal research assistant. Provide accurate relevance assessments."},
            ...
            return {"relevance_score": 0.0, "reasoning": "GPT analysis unavailable", "source": "error"}
