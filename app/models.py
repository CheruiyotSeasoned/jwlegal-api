...
class User(Base):
    ...
    research_submissions = relationship("ResearchSubmission", back_populates="user",foreign_keys="[ResearchSubmission.user_id]")
...
class CaseAssignment(Base):
    ...
    lawyer_id = Column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Document(Base):
    ...
    parent_document_id = Column(String(36), ForeignKey("documents.id", ondelete="SET NULL"))  # For document versions
...
class Message(Base):
    ...
    # Message metadata
    reply_to_message_id = Column(String(36), ForeignKey("messages.id", ondelete="SET NULL"))
    forwarded_from_message_id = Column(String(36), ForeignKey("messages.id", ondelete="SET NULL"))
...
class Invoice(Base):
    ...
    client_id = Column(String(36), ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
    lawyer_id = Column(String(36), ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class Payment(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    invoice_id = Column(String(36), ForeignKey("invoices.id", ondelete="CASCADE"), nullable=False, index=True)
    payer_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class ResearchSubmission(Base):
    ...
    # Relationships
    user = relationship("User", back_populates="research_submissions",foreign_keys="[ResearchSubmission.user_id]")
    case = relationship("Case", back_populates="research_submissions",foreign_keys="[ResearchSubmission.case_id]")
    responses = relationship("ResearchResponse", back_populates="submission", cascade="all, delete-orphan")
    
class ResearchResponse(Base):
    __tablename__ = "research_responses"
    
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    submission_id = Column(String(36), ForeignKey("research_submissions.id", ondelete="CASCADE"), nullable=False, index=True)
    researcher_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    content = Column(Text, nullable=False)
    sources = Column(JSON)
    attachments = Column(JSON)
    created_at = Column(DateTime, default=func.now())
